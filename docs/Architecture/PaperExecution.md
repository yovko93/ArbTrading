...
## Phase 04F deterministic adaptive paper sizing

FixedQuantity remains unchanged: one exact requested-quantity evaluation, no clipping or fallback. The separate `LargestAdmissibleGridQuantity` mode chooses the first fully admissible quantity in descending exact-decimal order `MinimumQuantity + N * QuantityStep <= MaximumQuantity`. It maximizes only configured grid quantity for one opportunity, not profit, ROI or portfolio allocation. There is no AI, forecast, Kelly sizing, live order or real-fund sizing. QuantityStep is a paper simulation grid step, not an exchange lot-size claim.

Positive minimum/step, minimum ≤ maximum ≤ saved risk quantity cap ≤ 1000, and at most 256 points are validated on save and arm. A 257-point grid is rejected, never truncated. Unaligned maximums are not invented as grid points. No monotonicity assumption, binary search, proportional fee/profit scaling or VWAP-based admission is used. Each tested Q uses the existing requested-quantity paired L2 walk, fee evaluator, PaperPlanner, PaperRiskEvaluator and automatic economics/session-budget checks. Larger risk, depth, cash reserve, session debit, edge or profit rejection permits trying the next smaller grid point. Quantity-independent missing source/quality/fee/relationship, kill, monitoring, invalid risk state, cooldown and execution-cap failures stop work.

Books are captured together; fee schedules/account profile are captured once and bound to the market stamp; a single deferred SQLite read transaction captures generation/risk/financial state and session history. Each candidate uses these immutable snapshots. Validation before and after search rejects changed books, relationship or fee proof, without a retry. At most 512 exact adaptive evaluations are allowed across one sequential worker cycle. A search starts only if the remaining budget can cover its entire configured grid; otherwise the key remains queued without being marked attempted. Unchanged completed attempts retain Phase 04E suppression. Existing FeeAdjusted ranking determines opportunity order; after a commit the next opportunity captures updated financial state.

Selection is advisory until the existing PaperStore writer transaction repeats kill, policy, history, current funds/risk and final runtime/monitor/book gates. A concurrent financial revision or risk rejection rejects the selected quantity. No lower-quantity retry or search occurs inside that financial attempt. Manual preview continues using its explicit quantity. Restart remains Disarmed, kill latch persists, and monitor/risk changes still disarm.

Adaptive profiles/proofs use version 2. Existing and newly saved fixed profiles keep version 1; null new grid fields are omitted from canonical settings JSON, preserving historical fingerprints. Legacy market-stamp and RequestId algorithms remain intact. Adaptive market stamps add mode, grid, profile revision/fingerprint but exclude cash, open exposure, session debit and P&L. Separate compact sizing proofs persist selected quantity, grid, evaluated count, bounded rejection counts, exact-plan SHA-256, risk/profile/generation/financial revisions, session debit state and decision SHA-256. Adaptive RequestIds bind the selected quantity and decision fingerprint. Reconciliation checks the historical recorded version and independent risk proof, never today's automation profile.

The existing profile/proof JSON columns hold these additive versioned fields; no schema migration is required or prior migration edited. No existing financial records are rewritten. `POST /paper/automation/sizing-preview` requires authenticated workspace membership, uses only the saved policy and local current inputs, and returns compact exact selected economics/risk/reasons. It performs no acquisition, financial or audit mutation, creates no manual preview ticket, and cannot arm. A disarmed preview uses an empty prospective session budget. While armed it uses that session's current history. Profile mutation and lifecycle controls remain owner-only.

WPF shows mode-dependent fields, an explicit adaptive arm warning, saved-profile diagnostics and historical selected quantity/grid/proof details. Suggested 1..25 step 1 values are inactive until saved. Status reports last sizing outcome/quantity/evaluation count and bounded counters for attempts, evaluations, selections, no-admissible, risk/depth/economics rejection, budget deferral and changed inputs. Per-quantity normal logging and rejected-plan persistence are intentionally absent.

## Phase 04E explicitly armed automatic paper execution

Auto Paper remains the Paper environment. An owner must explicitly save a workspace automation profile and confirm Arm against current automation/risk revisions, active healthy generation, and kill-switch revision. There are no active default settings and no startup arming. Monitoring must be Running. Phase 04E introduced FixedQuantity: every entry is recomputed at exactly that quantity through the existing opportunity, fee, depth and PaperPlanner path. Its insufficient depth/risk/funds behavior still rejects the entire attempt. The optional Phase 04F search described above occurs before the shared financial attempt. No automatic SELL, exit or settlement is performed.
...
Migration `20260923154728_PaperRiskPolicy` adds the policy table, nullable execution proof columns and open-state indexes. Earlier migrations are unchanged. Normal user storage must follow the documented explicit backup/upgrade procedure; development/tests migrate only isolated storage. Private exchange balances and real trading remain unavailable. Explicitly armed automatic paper entries and optional adaptive sizing are described above.
