...
public enum ConnectionState { Loading, Connected, Unavailable, AuthenticationFailed, Disconnected }
...
public sealed record BackendSnapshot(SessionResponse Session, SystemStatusResponse System,
    WorkspaceSettingsResponse Workspace, ExchangeStatusResponse[] Exchanges);
...
public sealed class BackendClient(HttpClient http, ILocalConnectionFile connections)
{
    public async Task<BackendSnapshot> LoadAsync(CancellationToken cancellationToken)
    {
        ...
        return new(session, status, workspace, exchanges);
    }
    ...
    private async Task<T> SendAsync<T>(HttpMethod method, string path, object? body, CancellationToken cancellationToken)
    {
        try
        {
            ...
            for (var attempt = 0; attempt < 2; attempt++)
            {
                ...
                return await response.Content.ReadFromJsonAsync<T>(cancellationToken) ?? throw new JsonException();
            }
