...
Phase 04E adds explicitly armed **Automatic Paper Execution** with fixed quantity, a persistent emergency kill switch, bounded monitoring consumption, durable deduplication, session/hourly/cooldown limits, and per-venue/currency gross debit budgets. Configure paper risk first, save an automation profile, start monitoring, then explicitly arm. Mode remains Paper; no real orders are submitted. The backend continues after WPF closes or disconnects. Backend restart always requires explicit rearming; resetting a kill switch never arms. See [paper architecture](docs/Architecture/PaperExecution.md) and [Phase 04E verification](docs/Development/Phase04EVerification.md).

Phase 04D introduced explicit, workspace-owned **Paper capital and risk policy**. Suggested form values remain inactive until saved. Both manual and automatic entries repeat admission inside the financial SQLite transaction. Existing settlement and valuation remain available; automation never bypasses risk admission or chooses an adaptive quantity.
