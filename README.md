...
Settlement records the payout vector and credits the original venue/currency bucket atomically. Settled positions retain entry cost and fees; executions become PartiallySettled or Settled. Realized P&L equals payout minus persisted fee-inclusive cost. Paper Performance exposes separate venue/currency cash history and a realized-performance curve: starting paper capital plus cumulative realized P&L. Open-position market values are excluded. No currency conversion or mark-to-market is implemented. Void, scalar, ambiguous, negative-risk and complex settlement are unsupported. Migration `20260923105901_PaperSettlement` is additive; follow the existing explicit backup/upgrade procedure. See [Phase 04B verification](docs/Development/Phase04BVerification.md).
