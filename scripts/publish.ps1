...
try {
    ...
    # A temporary archive supplies the extracted-only smoke. Promote to the deliverable only on success.
    $candidate = Join-Path $output ('.candidate-' + [Guid]::NewGuid().ToString('N') + '.zip')
    try {
        [IO.Compression.ZipFile]::CreateFromDirectory($stage, $candidate)
        & (Join-Path $PSScriptRoot 'verify-distribution.ps1') -PackageZip $candidate
        if ($LASTEXITCODE) { throw 'Distribution smoke failed.' }
        Move-Item -LiteralPath $candidate -Destination $zip
