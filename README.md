...
Phase 04D adds explicit, workspace-owned **Paper capital and risk policy**. Save and confirm a policy before creating new paper exposure; suggested form values are inactive until saved. Preview shows exact reserve, cost-basis exposure, concentration and count checks. Confirmation repeats admission inside the financial SQLite transaction and requires a fresh preview after policy or financial-state changes. Existing settlement and valuation remain available. No automatic sizing, automatic execution, or live orders are added. See [risk policy architecture](docs/Architecture/PaperExecution.md#phase-04d-capital-and-risk-admission) and [Phase 04D verification](docs/Development/Phase04DVerification.md).
...
Executions, native-level fills, proofs, cash debits and positions persist atomically in SQLite. Request IDs protect repeated confirmations after lost replies. Reset creates a new generation and retains old history. Expected payout/profit at resolution remains a projection until an explicit simulated settlement. Monitoring and alerts never execute automatically. Paper atomicity is **not** evidence that live cross-exchange execution can be atomic.
