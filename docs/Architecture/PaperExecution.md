...
Migration `20260923154728_PaperRiskPolicy` adds the policy table, nullable execution proof columns and open-state indexes. Earlier migrations are unchanged. Normal user storage must follow the documented explicit backup/upgrade procedure; development/tests migrate only isolated storage. Phase 04E, automatic sizing, automatic paper entries, private exchange balances, real trading and credentials remain outside this phase.
